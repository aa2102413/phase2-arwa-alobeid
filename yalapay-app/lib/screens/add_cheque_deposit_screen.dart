...
import 'package:yalapay/FB_Providers/cheque_check_provider.dart';
...
import '../FB_Providers/cheque_deposit_provider.dart';
import '../FB_Providers/cheque_provider.dart';

import '../FB_Providers/update_cheques_provider.dart';
...
void checkChequeStatus() {
  ...
  if (!watchChequeCheck) {
    var check = watchUpdatedCheques.where((e) => e.status == 'Cashed');
    if (check.length == watchUpdatedCheques.length && check.isNotEmpty) {
      readChequeCheckNotifier.toggle(); }}}

Future<void> validateLogin() async {
  final logged = await ref.read(loginProviderNotifier.notifier).isLoggedIn();
  if (!logged && mounted) {
    GoRouter.of(context).go(AppRouter.error.path);}}


  void setAmount(double value) {  setState(() {amount = value; });}
  void setCheques(List<Cheque> value) { setState(() {cheques = value;});}
  void getTotalChequeAmount() {
    double totalAmount = 0;
    if(cheques.isEmpty) { setAmount(0);return;}
    for (var cheque in cheques) {totalAmount += cheque.amount; }
  ...
  Widget build(BuildContext context) {
    ...
    final watchChequeDeposit = ref.watch(chequeDepositProvider);
    ...
    if(!widget.isAdd){
      ...
      for(var no in widget.chequeDeposit!.chequeNos){
         Cheque cheque =readChequeNotifier.getChequeById(no);
        temp += cheque.amount;
        watchSelectedCheques.add(cheque); }
      setAmount(temp);
    ...
    else{ getTotalChequeAmount(); }
    var amountText = amount?.toStringAsFixed(2) ?? '0.00';
    return Scaffold(
        backgroundColor: Colors.white,
        body: FutureBuilder(
          future: readChequeNotifier,
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [Expanded(
                                  child: widget.isAdd? Text("Cheque No's: ",
                ...
                onPressed:() {
                  ...
                  for(Cheque e in watchUpdatedCheques) {
                    if(e.status.toLowerCase() == 'deposited' && e.returnReason.isEmpty){
                      ...
                      e.status = 'Returned'; readChequeNotifier.updateCheque(e);
                    }else{  readChequeNotifier.updateCheque(e);
                    } }
                  ...
                  if(watchUpdatedCheques.isNotEmpty){
                    ...
                    for(var i in temp.chequeNos){
                        Cheque cheque = readChequeNotifier.getChequeById(i) as Cheque;
                      String status =  cheque.status;
                      if(status.toLowerCase() == 'cashed'){cashed+=1; }
                      else{ returned +=1;  }}
...
Future<void> validateLogin() async {
  ...
  Widget build(BuildContext context) {
    ...
    if(!widget.isAdd){
      ...
      for(var no in widget.chequeDeposit!.chequeNos){
         Cheque cheque =readChequeNotifier.getChequeById(no) as Cheque;
        temp += cheque.amount;
        watchSelectedCheques.add(cheque); }
      setAmount(temp);
      selectedBankAccount = widget.chequeDeposit!.bankAccountNo;
