...
import '../FB_Providers/invoice_provider.dart';
import '../FB_Providers/payment_provider.dart';
import '../models/cheque.dart';

class AccordianPayment extends ConsumerStatefulWidget {
  final Payment payment;
  final bool isInvoice;
  const AccordianPayment(
      {super.key, required this.payment, required this.isInvoice});
...
class _AccordianPaymentState extends ConsumerState<AccordianPayment> {
  ...
  Widget build(BuildContext context) {
    final readInvoiceNotifier = ref.read(invoiceNotifierProvider.notifier);
    ...
    final chequeFuture =
        widget.payment.paymentMode.toLowerCase() == 'cheque'.toLowerCase()
            ...
            : null;
    final invoiceFuture = readInvoiceNotifier.getInvoiceById(widget.payment.invoiceNo);
      ...
      child:  FutureBuilder<Invoice?>(
        ...
        builder: (context, invoiceSnapshot) {
          if (invoiceSnapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Text('Error loading invoice details.');
          }
          ...
          return FutureBuilder<Cheque?>(
            ...
            builder: (context, chequeSnapshot) {
              if (chequeSnapshot.connectionState == ConnectionState.waiting) {
                ...
                return const Text('Error loading cheque details.');
              }
                        ...
                        widget.payment.paymentMode.toLowerCase() == 'cheque'.toLowerCase()
                            ...
                            : Flexible(
                                ...
                                child: Container(
                                  ...
                                  child: Text('${widget.payment.paymentMode}.',
                                      style: TextStyle(
  ...
  Widget buildChequeDetails(cheque, invoice) {
    ...
    future: Future.wait([
      readChequeNotifier.getChequeById(widget.payment.chequeNo),
      readInvoiceNotifier.getInvoiceById(widget.payment.invoiceNo),
    ] as Iterable<Future>),
    builder: (context, AsyncSnapshot<List<dynamic>> snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) {
        ...
        final cheque = snapshot.data![0];
        final invoice = snapshot.data![1];
                                ...
                                child: Row(
                                  children: [
                                    Container(
                                      ...
                                      child: cheque.status.isNotEmpty
                                          ? Text(
                                              cheque.status,
                                              style: TextStyle(
                                                  color: cheque.status == 'Cashed'
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          SimpleDate(cheque.receivedDate.year,
                                  cheque.receivedDate.month, cheque.receivedDate.day)
                              .toString(),
                          style: TextStyle(fontSize: 12),
                        ),
                        const SizedBox(height: 25),
                      ...
                      if (!widget.isInvoice) {
                        router.go(
                            ...
                            extra: PaymentArgs(
                                ...
                                invoiceId: widget.payment.invoiceNo,
                          ...
                          builder: (context) {
                            return AlertDialog(
                              ...
                              actions: [
                                ...
                                TextButton(
                                  onPressed: () {
                                    if (widget.payment.paymentMode.toLowerCase() ==
                                        'cheque'.toLowerCase()) {
                                      readChequeNotifier.deleteCheque(
                                          readChequeNotifier .getChequeById(widget.payment.chequeNo));

                                      readChequeDepositNotifier.updateChequeDepositOnChequeDelete(
                                              widget.payment.chequeNo);
                                    }
